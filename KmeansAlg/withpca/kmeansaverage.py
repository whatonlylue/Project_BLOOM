...
# Apply PCA to reduce tensor representations to 2D
pca = PCA(n_components=2)
reduced_tensors = pca.fit_transform(tensor_representations.cpu().numpy())  # Ensure tensor is on CPU
...
cluster_labels = kmeans.fit_predict(reduced_tensors)

# Plot tensor-level clusters
plt.scatter(reduced_tensors[:, 0], reduced_tensors[:, 1], c=cluster_labels, cmap='tab10', alpha=0.7)
